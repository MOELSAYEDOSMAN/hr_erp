...
namespace EmployeeERP.API.Service.UnitOfWork
{
    public class UnitService<T>(IMemoryCacheCustomizeService<T> _memoryCache) : IUnitService<T> where T : BaseModel
    {
        ...
        {
            var filter_list = _memoryCache.GetData().Where(d=>d.soft_delete==false).Where(item => predicate(item, input)).OrderByDescending(d=>d.modified);
        ...
        {
            var data = _memoryCache.GetData().Where(d=>d.soft_delete==false).OrderByDescending(d=>d.modified);
        ...
        async Task<T> IUnitService<T>.Insert(T input, string? owner)
        {
            ...
            input.modified = DateTime.UtcNow;
